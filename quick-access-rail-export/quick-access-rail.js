...
(function () {
  ...
  // ---------- opsiyonel: dil değiştirme (.qar-rail-lang / [data-qar-lang-toggle]) ----------
  // Kendi i18n sisteminiz varsa 'qar:langchange' event'ini dinleyip metinleri kendiniz güncelleyin:
  //   document.addEventListener('qar:langchange', (e) => applyMyOwnTranslations(e.detail.lang));
  document.querySelectorAll('[data-qar-lang-toggle]').forEach((toggle) => {
    toggle.querySelectorAll('button').forEach((btn) => {
      btn.addEventListener('click', () => {
        toggle.querySelectorAll('button').forEach((b) => b.classList.toggle('active', b === btn));
        document.dispatchEvent(new CustomEvent('qar:langchange', { detail: { lang: btn.dataset.lang } }));
      });
    });
  });
})();
